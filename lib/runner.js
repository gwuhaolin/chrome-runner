...
class Runner {
  ...
  prepareChromeDataDir() {
    ...
    this.chromeOutPath = `${this.chromeDataDir}/chrome-out.log`;
    this.chromeOutFd = fs.openSync(this.chromeOutPath, 'a');

    this.chromeErrorPath = `${this.chromeDataDir}/chrome-err.log`;
    this.chromeErrorFd = fs.openSync(this.chromeErrorPath, 'a');

    this.pidPath = `${this.chromeDataDir}/chrome.pid`;
    this.pidFd = fs.openSync(this.pidPath, 'w');
  ...
  destroyChromeDataDir() {
    if (this.chromeOutFd) {
      ...
      fs.unlinkSync(this.chromeOutPath);
      delete this.chromeOutFd;
      delete this.chromeOutPath;
    }

    if (this.chromeErrorFd) {
      fs.closeSync(this.chromeErrorFd);
      fs.unlinkSync(this.chromeErrorPath);
      delete this.chromeErrorFd;
      delete this.chromeErrorPath;
    }
    ...
    const DevToolsActivePort = path.resolve(this.chromeDataDir, 'DevToolsActivePort')
    if (fs.existsSync(DevToolsActivePort)) {
      fs.unlinkSync(DevToolsActivePort);
    }

    fs.rmdirSync(this.chromeDataDir);
